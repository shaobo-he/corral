...
namespace cba.Util
{
    ...
    public class RemoveVarsFromAttributes : FixedVisitor
    {
        ...
        {
            locals = new HashSet<string>();
            foreach (var v in node.LocVars) locals.Add(v.Name);
        ...
        private QKeyValue Remove(QKeyValue attr, HashSet<string> vars)
        {
            ...
            var vu = new VarsUsed();
            foreach (var e in attr.Params.Where(e => e is Expr)) vu.VisitExpr(e as Expr);
    ...
    public class ExprMatchVisitor : FixedVisitor
    {
        ...
        public override Expr VisitNAryExpr(NAryExpr node)
        {
                ...
                && BoogieUtil.checkAttrExists(BoogieKeyWords.AnyExpr, ((FunctionCall)((NAryExpr)_toConsume.Peek()).Fun).Func.Attributes))
            {
                ...
                _toConsume.Pop();
                foreach (var arg in ((NAryExpr)_toConsume.Peek()).Args.AsEnumerable().Reverse()) _toConsume.Push(arg);
            ...
            {
                _toConsume.Pop();
                foreach (var arg in naeToConsume.Args.AsEnumerable().Reverse()) _toConsume.Push(arg);
                return base.VisitNAryExpr(node);
            }
        ...
        public override Expr VisitIdentifierExpr(IdentifierExpr node)
        {
                ...
                && BoogieUtil.checkAttrExists(BoogieKeyWords.AnyExpr, ((FunctionCall)((NAryExpr)_toConsume.Peek()).Fun).Func.Attributes))
            {
                ...
                _toConsume.Pop();
                foreach (var arg in ((NAryExpr)_toConsume.Peek()).Args) _toConsume.Push(arg);
        ...
        public class BoogieKeyWords
        {
			...
			// match pointer references before applying parameter rule
			public const string MatchPtrs = "#MatchPtrs";
			//procedure to call in CorralExtraInit
			public const string CorralExtraInitExtn = "corralExtraInitExtension";

            public static string[] AllKeywords = { AnyParams, AnyArgs, AnyExpr, AnyLhss, AnyType, AnyProcedure, IdExpr, NameMatches, NameNotMatches, NoImplementation, ReplaceImplementation, CorralExtraInitExtn, MatchPtrs };
        }
    }
}
